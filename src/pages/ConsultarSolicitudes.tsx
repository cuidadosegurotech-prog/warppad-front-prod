...
import { formatDistanceToNow, differenceInDays, parseISO, differenceInHours, differenceInMinutes, parse } from "date-fns";
import { es } from "date-fns/locale";
import { useToast } from "@/hooks/use-toast";
import { useKeycloak } from "@/context/KeycloakContext";
import ModalDetalle from '@/components/SolicitudesDetalle';
import ModalUpdate from '@/components/ModificarSolicitud';
...
export default function ConsultarSolicitudes() {
  ...
  const getTimeElapsed = (fechaCreacion: string, fechaActualizacion: string) => {
    try {
      const vFechaCreacionSeparada = fechaCreacion.split('/');
      const vFechaActualizacionSeparada = fechaActualizacion.split('/');
      const dfechaCreacion = new Date(`${vFechaCreacionSeparada[1]}/${vFechaCreacionSeparada[0]}/${vFechaCreacionSeparada[2]}`);
      const dFechaActualizacion = new Date(`${vFechaActualizacionSeparada[1]}/${vFechaActualizacionSeparada[0]}/${vFechaActualizacionSeparada[2]}`)
      const minutosTranscurridos = differenceInMinutes(dFechaActualizacion, dfechaCreacion);

      const dias = Math.floor(minutosTranscurridos / (60 * 24));
      const horas = Math.floor((minutosTranscurridos % (60 * 24)) / 60);
  ...
  const handleExportData = () => {
    ...
    const data = filteredSolicitudes.map(solicitud => ({
      ...
      EPS: solicitud.ClinicaPrestadora,
      Paciente: solicitud.DatosPaciente,
    ...
    XLSX.writeFile(
      workbook,
      `solicitudes_${new Date().toISOString().split("T")[0]}.xlsx`
    );
  };
          ...
          <Table>
            <TableHeader className="sticky top-0 z-10">
              <TableRow className="hover:bg-transparent border-slate-200 bg-slate-50">
                ...
                <TableHead className="text-slate-700 font-semibold py-3 text-sm">Paciente</TableHead>
              ...
              {currentSolicitudes.length === 0 ? (
                ...
                currentSolicitudes.map((solicitud) => (
                  ...
                  >
                    ...
                    <TableCell className="font-medium text-slate-800 max-w-xs py-3">
                      <div className="truncate text-sm" title={solicitud.DatosPaciente}>
                        {solicitud.DatosPaciente}
                      </div>
                    </TableCell>
                     <TableCell className="font-medium text-slate-800 max-w-xs py-3">
                    ...
                    <TableCell className="py-3">
                      <span className="text-sm text-slate-700">{solicitud.ClinicaPrestadora}</span>
                    </TableCell>
                    <TableCell className="py-3">
                      <div className="flex items-center gap-2">
                        {getStatusIcon(solicitud.estado)}
  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="container mx-auto px-4 py-6 max-w-7xl space-y-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
          {[
            ...
            { label: "Completadas", value: solicitudes.filter(s => s.estado === "Completada").length, color: "text-green-600" },
            { label: "En progreso", value: solicitudes.filter(s => s.estado === "Proceso").length, color: "text-yellow-600" },
      ...
      {/* Botón de Feedback removido de aquí - ahora está en MainLayout como flotante global */}
    </div>
  );
}
